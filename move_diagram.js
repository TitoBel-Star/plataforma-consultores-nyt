...
// 1. Remove the old diagram from the #contacto section
const regex = /<!-- Diagrama de Ecosistema NyTEX -->[\s\S]*?<\/div>(\s*)<div class="flex flex-col sm:flex-row justify-center gap-6">/;
html = html.replace(regex, '$1<div class="flex flex-col sm:flex-row justify-center gap-6">');

// 2. Insert the diagram right under the line in the "ECOSISTEMA" section
const targetString = '<div class="w-24 h-1 bg-brand mx-auto"></div>\n            </div>\n\n            <div class="grid grid-cols-1';
html = html.replace(targetString, '<div class="w-24 h-1 bg-brand mx-auto"></div>\n            </div>\n' + diagramHTML);

fs.writeFileSync('index.html', html);
console.log("Moved diagram under ECOSISTEMA successfully.");
